 ...
 * Last modified: 2020.05.23 at 15:06:15
...
class FavoriteDataTable extends DataTable
{
    ...
    public function query(Favorite $model)
    {
        ...
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("clothes")->with("user")->select("favorites.*");
        } else {
            return $model->newQuery()->with("clothes")->with("user")
                ->join("clothes","clothes.id","=","favorites.clothes_id")
    ...
    protected function getColumns()
    {
        $columns = [
            [
                'data' => 'clothes.name',
                'title' => trans('lang.favorite_clothes_id'),
