...
class ExtraDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('clothes.name', function ($extra) {
                return getLinksColumnByRouteName([$extra->clothes->toArray()], 'clothes.edit','id','name');
            })
            ->editColumn('clothes.shop.name', function ($extra) {
                return getLinksColumnByRouteName([$extra->clothes->shop->toArray()], 'shops.edit', 'id', 'name');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            [
                'data' => 'clothes.name',
                'title' => trans('lang.clothes'),

            ],
            [
                'data' => 'clothes.shop.name',
                'title' => trans('lang.shop'),
    ...
    public function query(Extra $model)
    {
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("clothes")->with("extraGroup")->with('clothes.shop');
        } else if (auth()->user()->hasRole('manager')) {
            return $model->newQuery()->with("clothes")->with("extraGroup")->with('clothes.shop')
                ->join("clothes", "extras.clothes_id", "=", "clothes.id")
                ->join("user_shops", "clothes.shop_id", "=", "user_shops.shop_id")
