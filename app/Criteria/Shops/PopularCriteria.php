    ...
    public function apply($model, RepositoryInterface $repository)
    {
        if ($this->request->has(['myLon', 'myLat', 'areaLon', 'areaLat'])) {
            ...
            return $model->select(DB::raw("SQRT(
                ...
                POW(69.1 * ($areaLon - longitude) * COS(latitude / 57.3), 2))  AS area count(shops.id) as shop_count"), "shops.*")
            ...
            return $model->select(DB::raw("count(shops.id) as shop_count"), "shops.*")
                ->join('clothes', 'clothes.shop_id', '=', 'shops.id')
                ->join('clothes_orders', 'clothes.id', '=', 'clothes_orders.clothes_id')
                ->orderBy('shop_count', 'desc')
                ->groupBy('shops.id');
