...
class EarningDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ->editColumn('shop.name', function ($earning) {
                return getLinksColumnByRouteName([$earning->shop], "shops.edit",'id','name');
            })
            ...
            ->editColumn('shop_earning', function ($earning) {
                return getPriceColumn($earning,'shop_earning');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            [
                'data' => 'shop.name',
                'title' => trans('lang.earning_shop_id'),
            ...
            [
                'data' => 'shop_earning',
                'title' => trans('lang.earning_shop_earning'),
    ...
    public function query(Earning $model)
    {
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with("shop")->select('earnings.*');
        }else if((auth()->user()->hasRole('manager'))){
            return $model->newQuery()->with("shop")
                ->join("user_shops", "user_shops.shop_id", "=", "earnings.shop_id")
                ->where('user_shops.user_id', auth()->id())->select('earnings.*');
        }
    }
