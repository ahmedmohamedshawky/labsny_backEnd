...
class SlideController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('slides.create')->with("customFields", isset($html) ? $html : false)->with("clothes", $clothes)->with("shop", $shop);
    }
    ...
    {
        $slide = $this->slideRepository->findWithoutFail($id);
        $clothes = $this->clothesRepository->pluck('name', 'id')->toArray();
        $shop = $this->shopRepository->pluck('name', 'id')->toArray();
        $clothes = array('' => trans('lang.slide_clothes_id_placeholder')) + $clothes;
        $shop = array('' => trans('lang.slide_shop_id_placeholder')) + $shop;
        ...
        return view('slides.edit')->with('slide', $slide)->with("customFields", isset($html) ? $html : false)->with("clothes", $clothes)->with("shop", $shop);
    }
    ...
    public function update($id, UpdateSlideRequest $request)
    {
        ...
        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->slideRepository->model());
        try {
            if(!isset($input['clothes_id'])){
                $input['clothes_id'] = null;
            }
