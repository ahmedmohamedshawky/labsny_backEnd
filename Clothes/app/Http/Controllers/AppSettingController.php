...
class AppSettingController extends Controller
{
    ...
    public function __construct(UserRepository $userRepo, RoleRepository $roleRepo, UploadRepository $uploadRepository, 
                                CurrencyRepository $currencyRepository, CoinRepository $coinRepo)
    {
        parent::__construct();
...
//                }
            } else if (Str::contains(url()->previous(), "coins")) {
                $coinRepository = $this->coinRepository->first();
                $this->coinRepository->update($request->all(), $coinRepository->id);
            }
    ...
    public function index($type = null, $tab = null)
    {
        ...
        $coinStructure = $this->coinRepository->first();
        ...
        return view('settings.' . $type . '.' . $tab . '', compact(['languages', 'type', 'tab', 'langFiles', 'timezones', 'upload', 'customFieldModels', 'currencies', 'mobileLanguages', 'containsUpdate', 'coinStructure']));
    }
