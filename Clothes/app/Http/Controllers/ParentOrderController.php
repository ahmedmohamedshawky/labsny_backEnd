...
abstract class ParentOrderController extends Controller
{
    ...
    protected $clothesOrderRepository;
    /** @var  CartRepository */
     ...
     */
    public function __construct(OrderRepository $orderRepo, ClothesOrderRepository $clothesOrderRepository, CartRepository $cartRepo, PaymentRepository $paymentRepo, NotificationRepository $notificationRepo, UserRepository $userRepository, OrderStatusRepository $orderStatusRepository, CouponRepository $couponRepository)
    {
        parent::__construct();
    ...
    protected function calculateTotal(): float
    {
        ...
        foreach ($carts as $_cart) {
            $price = $_cart->clothes->applyCoupon($this->coupon);
    ...
    protected function calculateDeliveryFee(): void
    {
        try {
            $carts = $this->order->user->cart;
            $this->order->delivery_fee = $carts[0]->clothes->shop->delivery_fee;
    ...
    protected function calculateTaxFee(): void
    {
        try {
            $carts = $this->order->user->cart;
            if (empty($carts[0]->clothes->shop->default_tax)) {
                $this->order->tax = setting('default_tax', 0);
            } else {
                $this->order->tax = $carts[0]->clothes->shop->default_tax;
            }
    ...
    protected function syncClothes()
    {
        ...
        foreach ($this->order->user->cart as $cart) {
            $clothes = [
                ...
                'price' => $cart->clothes->applyCoupon($this->coupon),
                'quantity' => $cart->quantity,
                'options' => $cart->extras->pluck('id')->toArray(),
            ];
            try {
                $this->clothesOrderRepository->create($clothes);
            } catch (ValidatorException $e) {
            }
    ...
    {
        Notification::send($this->order->user->cart[0]->clothes->shop->users, new NewOrder($this->order));
    }
