...
use App\Criteria\Users\DriversOfShopCriteria;
...
class OrderController extends Controller
{
    ...
    public function store(CreateOrderRequest $request)
    {
        ...
        try {
            ...
            //     $input['tax'] = auth()->user()->commissions / 100;
    ...
    public function show(ClothesOrderDataTable $clothesOrderDataTable, $id)
    {
        ...
        $total += $taxAmount;
        $clothesOrderDataTable->id = $id;

        return $clothesOrderDataTable->render('orders.show', ["order" => $order, "total" => $total, "subtotal" => $subtotal,"taxAmount" => $taxAmount]);
    }
    ...
    public function edit($id)
    {
        ...
        $shop = $order->clothesOrders()->first();
        $shop = isset($shop) ? $shop->clothes['shop_id'] : 0;

        $user = $this->userRepository->getByCriteria(new ClientsCriteria())->pluck('name', 'id');
        $driver = $this->userRepository->getByCriteria(new DriversOfShopCriteria($shop))->pluck('name', 'id');
        $orderStatus = $this->orderStatusRepository->pluck('status', 'id');
